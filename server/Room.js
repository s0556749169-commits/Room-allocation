...
app.get('/api/available-rooms', async (req, res) => {
  try {
    const { wing, floor, minSize, hasProjector, date, startTime, endTime } = req.query;

    if ((date || startTime || endTime) && (!date || !startTime || !endTime)) {
      return res.status(400).json({ message: 'לחיפוש זמינות חובה לשלוח date, startTime, endTime' });
    }
    ...
    if (hasProjector !== undefined) filter.hasProjector = hasProjector === 'true';

    const rooms = await Room.find(filter).sort({ size: 1, floor: 1, wing: 1 });
    ...
    if (!isValidTimeFormat(startTime) || !isValidTimeFormat(endTime) || startTime >= endTime) {
      return res.status(400).json({ message: 'שעות לא תקינות. יש לשלוח בפורמט HH:mm' });
    }
...
const cancelledRoomIds = new Set(
  cancellations.map(item => item.roomId.toString())
    ...
    const conflicts = await Allocation.find({
      ...
      endTime: { $gt: startTime },
      $or: [
        { type: 'temporary', date: { $gte: selectedDate, $lt: nextDate } },
        { type: 'permanent', dayOfWeek }
      ]
    });
